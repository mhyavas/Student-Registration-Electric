                ...
                (dom/div
                  (ui4/button (e/fn []
                                    (e/server (dt/transact conn {:tx-data [{:author/id (next-author-id db)
                                                                            ...
                                                                            :author/company (ffirst (dt/q '[:find ?e :in $ ?name :where [?e :customer/name ?name]] db name))}]})))
                  ...
                  (dom/div
                    ...
                    (dom/select
                      (e/for [value (e/server (flatten (dt/q '[:find ?name
                             ...
                             (dom/option (dom/props {:value value})
                                         (dom/text value)))
                      (dom/on "change" (e/fn [e] (swap! !state-project assoc-in [:msg/author] (.-value dom/node))))))
                  (dom/div
                    (dom/table
                      ...
                      (e/for [value (e/server (dt/q '[:find (pull ?e [*])
                                                      ...
                                                      :where [?e :project/customer ?name]
                                                          [?e :project/status :inactive]] db (ffirst (dt/q '[:find ?e :in $ ?name :where [?e :customer/name ?name]] db info))))]
                             (dom/tr
                               (dom/td (dom/text (:project/title (first value))))
                               (dom/td (history/link [::chat [(:project/title (first value)) (:msg/author state)]]
                                                     (dom/text (e/server (ffirst (dt/q '[:find ?name
                                                                                         :in $ ?title
                                                                                         :where [?p :project/title ?title]
                                                                                                [?ppl :proposal/project ?p]
                                                                                                [?ppl :proposal/supplier ?s]
                                                                                                [?s :supplier/name ?name]] db (:project/title (first value))))))))))))))))))
              ...
              (e/client
                (let [state (e/watch !state-project)]
                  ...
                  #_(dom/text (e/server (dt/q '[:find (pull ?e [*])
                                                ...
                                                [?e :msg/id ?id]] (dt/db conn) title)))
                  (dom/ul
                    (e/for [msg (e/server (dt/q '[:find (pull ?e [*])
                                                  :in $ ?title
                             ...
                             (if (nil? (:msg/reply_to (first msg)))
                               (dom/div
                                 (dom/text (ffirst (e/server (dt/q '[:find ?name
                                                                     :in $ ?id
                                                                     :where [?id :author/name ?name]] (dt/db conn) (:db/id (:msg/author (first msg)))))) " -> " (:msg/message (first msg)) " at " (e/server (if (and (and (= (.getMonth (java.util.Date.)) (.getMonth (java.util.Date. (:msg/timestamp (first msg)))))
                                 ...
                                 (ui4/button (e/fn []
                                                   (swap! !state-project assoc-in [:msg/reply] (:db/id (first msg)))) (dom/text "Reply")))


                               (dom/div
                                 (def msg-t (e/server (ffirst (dt/q '[:find (pull ?reply-id [*])
                                                                      :in $ ?reply-id
                                                                      :where [?reply-id :msg/id _]] (dt/db conn) (:db/id (:msg/reply_to (first msg))))))) (:msg/timestamp (e/client msg-t))
                                 (dom/text (ffirst (e/server (dt/q '[:find ?name
                                                                     :in $ ?id
                                                                     :where [?id :author/name ?name]] (dt/db conn) (:db/id (:msg/author (e/client msg-t) ))))) " -> " (:msg/message msg-t) " at " (e/server (if (and (and (= (.getMonth (java.util.Date.)) (.getMonth (java.util.Date. (:msg/timestamp (e/client msg-t)))))
                               ...
                               (dom/on "keydown" (e/fn [e]
                                                       (when (= "Enter" (.-key e))
                                                         (when-some [v (empty->nil (.substr (.. e -target -value) 0 100))]
                                                           ...
                                                           (swap! !state-project assoc-in [:msg/reply] 0)
                                                           (set! (.-value dom/node) "")))))))))))))
