...
(defn general-setter! [field value]
    (swap! !state assoc-in [:stage-student (keyword (last (clojure.string/split field #"_")))] value))
              ...
              (e/client
                ...
                (let [state (e/watch !state)]
                  (let [stage (:stage-student state)]
                    (dom/div
                      (e/for [field-name (e/server (map (fn [m] (:field/description m)) (->> (flatten (dt/q '[:find (pull ?e [:field/status :field/description])
                                                                                                              :where [?e :field/status :active]] db))
                                                                                            (sort-by :field/description))))]
                             (if (clojure.string/includes? field-name "department")
                               (dom/div
                               ...
                               (dom/div
                                 ...
                                 (ui4/input ((keyword (last (clojure.string/split field-name #"_"))) stage) (e/fn [v]
                                                                               (general-setter! field-name v)
                                                                               #_((resolve ((ns-publics app.todo-list) (symbol "set-name!"))) v)
                                                                               #_(set-name! v)))))
                             ))
                    #_(dom/span  (dom/text "Name:"))
                    #_(ui4/input (:name stage) (e/fn [v] (set-name! v)))
                    #_(dom/div
                    ...
                    #_(dom/span  (dom/text "Course:"))
                    #_(ui4/input (:course stage) (e/fn [v] (set-course! v)))
