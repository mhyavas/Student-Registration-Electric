...
(defn authenticate [username password] username)
(defn wrap-demo-router "A basic path-based routing middleware"
  [next-handler]
  (fn [ring-req]
    (case (:uri ring-req)
      ;"/test"  (res/created "/test" "TEST PAGE") ; redirect to where the auth originated
...
(defn wrap-index-page [next-handler resources-path manifest-path]
  (fn [ring-req]
    (if-let [response (res/resource-response (str resources-path "/index.html"))]
      ...
      (if-let [response (res/resource-response (str resources-path "/test.html"))]
        (if-let [modules (get-modules manifest-path)]
          (do
            (println modules)
           (-> (res/response (template (slurp (:body response)) modules)) ; TODO cache in prod mode
