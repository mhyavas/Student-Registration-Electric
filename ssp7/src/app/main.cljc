...
(def !state-main (atom {:customer {:name ""}
                        :supplier {:name ""
                                   :types []}}))
(e/def conn)
...
(defn set-supplier-name! [name]
  (swap! !state-main assoc-in [:supplier :name] name))
                     ...
                     (dom/input (dom/props {:type  "checkbox"
                                ...
                                (dom/on "change" (e/fn [v]
                                                       (if (and (.-checked dom/node) (not (some (partial = (str (.-name dom/node))) (:types supplier))))
                                                         ...
                                                         (swap! !state-main update-in [:supplier :types] conj (str (.-name dom/node)))
                                                         (if (some (partial = (str (.-name dom/node))) (:types supplier)))
                                                         (swap! !state-main update-in [:supplier :types] (fn [types] (vec (remove #(= (str (.-name dom/node)) %) types))))
                                                         nil))))
      ...
      (let [state (e/watch !state-main)]
        (let [customer (:customer state)]
          (dom/div
            ...
            (ui4/input (:name customer) (e/fn [v] (swap! !state-main assoc-in [:customer :name] v))))
...
(e/defn MainPage []
  (e/client
    ...
    ;todo Create Supplier ve Create Customer sayfalari super admin sayfasina tasinacak
    (dom/div
...
(e/defn Page [[page x]]
  (e/client
    ...
    (dom/div
      (dom/ul (dom/props {:class "ul"})
              ...
              (dom/li (dom/props {:float "left"}) (history/link [::customer-company-select] (dom/text "Customer Side")))
  ...
  (case page
    ...
    ::customer-company-select (history/router 1 (e/server (app.customer/CompanySelect.)))
    ...
    ::customer-review (history/router 2 (e/server (app.customer/Reviews. x)))
