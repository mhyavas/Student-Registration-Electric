...
#?(:clj (defn init-conn [schema1 schema2 schema3]
          (let [uri "datomic:mem://students"]
            (d/delete-database uri)
            (d/create-database uri)
            (let [conn (d/connect uri)]
              (d/transact conn schema1)
              (d/transact conn schema2)
              (d/transact conn schema3)
              (when !taker (!taker))
...
#?(:clj (defonce !conn (init-conn schema-dept schema-course schema-student)))
#?(:clj (comment (alter-var-root #'!conn (fn [_] (init-conn schema-dept schema-course schema-student)))))


(e/def db)                                                  ; server
(e/def transact!) ; server
#_(def !state #?(:cljs (atom "")))
...
#_(e/defn CreateStudent []
        (dom/span (dom/props {:class "input-load-mask"})
                  (dom/on-pending (dom/props {:aria-busy true})
                                  (dom/input
                                    (ui/on-submit (e/fn [m]
                                                        (e/server (transact! [(strToMap m)]) nil)))
                                    (dom/props {:placeholder "Student Record (:student/id 1 :student/name Ali Veli :student/department :department/matematik"})))))
